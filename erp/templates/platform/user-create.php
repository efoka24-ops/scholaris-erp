...
<div class="page-header">
    <div>
        ...
        <p class="subtitle">
            Ce compte n'appartiendra a aucun établissement et pourra instruire
...
<div class="card">
    ...
    <form method="post" action="/admin/comptes">
        ...
        <h2 style="margin-top:1.5rem">Périmètre</h2>
        <p class="muted" style="margin-top:-0.5rem">
            Ce que ce compte pourra voir. Le périmètre se choisit ici, et non
            après coup : créer un compte national puis le restreindre laisse
            entre les deux une fenetre ou il voit tout.
        </p>

        <div class="grid-2">
            <div class="field">
                <label for="scope_type">Étendue</label>
                <select id="scope_type" name="scope_type">
                    ...
                    <option value="MINISTRY" <?= $value('scope_type') === 'MINISTRY' ? 'selected' : '' ?>>
                        Une tutelle — administrateur de ministère
                    </option>
                    <option value="REGION" <?= $value('scope_type') === 'REGION' ? 'selected' : '' ?>>
                        Une region — délégué regional
                    </option>
                    <option value="DEPARTMENT" <?= $value('scope_type') === 'DEPARTMENT' ? 'selected' : '' ?>>
                        Un département — délégué départemental
                    </option>
                </select>
            </div>
            <div class="field">
                <label for="scope_value">Tutelle, region ou département</label>
                ...
                <datalist id="scope-values">
                    ...
                    <?php foreach ($departments as $department) : ?>
                        <option value="<?= $this->e($department) ?>">Département <?= $this->e($department) ?></option>
                    <?php endforeach; ?>
                </datalist>
            </div>
        </div>
        ...
        <p class="muted">
            Un délégué consulte son territoire ; il n'administre pas la
            plateforme et ne créé pas d'établissement.
        </p>
