...
<p class="subtitle">
    Une seule année est activé, et une seule séquence ouverte a la saisie.
    Tant qu'aucune séquence n'est ouverte, ni notes ni appel ne peuvent etre
    enregistrés.
</p>

<?php if ($years === []) : ?>
    <div class="alert alert--error">
        Aucune année scolaire n'est encore ouverte. Creez-en une pour commencer :
    ...
    <div class="card">
        <div class="inline-form" style="justify-content:space-between;align-items:center">
            ...
            <?php if (! $isActive && $rbac->allows('academic-years:update')) : ?>
                <form method="post" action="/annees-scolaires/<?= $this->e($year['id']) ?>/activer" class="inline-form">
                    <input type="hidden" name="_token" value="<?= $this->e($csrfToken) ?>">
                    <button type="submit" class="button button--secondary">Rendre activé</button>
                </form>
            <?php endif; ?>
        </div>
        ...
        <table class="table" style="margin-top:1rem">
            ...
            <?php foreach ($periods[(string) $year['id']] ?? [] as $period) : ?>
                ...
                <tr>
                    <td>Séquence <?= (int) $period['number'] ?></td>
